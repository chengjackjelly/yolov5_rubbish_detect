...
class LoadPrePost:
    def __init__(self, preimg,postimg, img_size=640, stride=32, auto=True):
        self.img_size = img_size
        self.stride = stride
        self.preimg=preimg
        self.postimg=postimg
        self.imgs=[preimg,postimg]
        self.mode = 'image'
        self.auto = auto
        assert preimg is not None and postimg is not None
        self.nf=2
    def __iter__(self):
        self.count = 0
        return self
    ...
    def __next__(self):
        if self.count == self.nf:
            raise StopIteration
        imgread = self.imgs[self.count]
        imBytes = np.frombuffer(imgread, np.uint8)
        img0 = cv2.imdecode(imBytes, cv2.IMREAD_COLOR)
        # Read image
        self.count += 1
        ...
        if(self.count==1):
            ...
            s = f'pre image {self.count}/{self.nf} : '
        elif(self.count==2):
            s = f'post image {self.count}/{self.nf} : '
        # Padded resize
